...
public class PlayerControl : MonoBehaviour
{
	...
	void Update()
    {
        ...
        //}
        int id = 0;
        float axis = Input.GetAxisRaw("joystick 1 axis 1");
        if(axis != 0.0f)
        {
            Debug.Log("plsno");
        }
        id = GameLoop.Instance.m_gameplayVariables.m_controls.GetDownOnAnyController(EKeyId.EKeyId_Confirm);
        if(id != -1)
        {
            Debug.Log(id);
        }
        ...
        {
            float t = m_timeUntillAction1DelayTimerASFloatingPointMightAlsoPossbilyBeSetToZeroIfYouDontWantADelay / playerVars.m_waveSpawnDelay;
